...
void	move(t_data *data)
{
	...
	if (data->mov.backward)
		handle_movement(XK_S, data);
	if (data->mov.right)
		handle_movement(XK_D, data);
	if (data->mov.left)
		handle_movement(XK_A, data);
	if (data->mov.turn_l)
		handle_rotation(XK_Left, data);
	if (data->mov.turn_r)
		handle_rotation(XK_Right, data);
}
