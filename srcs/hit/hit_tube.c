...
{
	const double	max_dist_squared = tube->half_height * tube->half_height
	...
	t_interval interval)
{
	...
	const t_vec3	oc = vec3_sub(ray->origin, tube->center);
	const double	a = vec3_length_squared(new_dir);
	const double	half_b = vec3_dot(new_dir, vec3_cross(oc, tube->axis));
	double			discriminant;
	...
	t_interval interval)
{
	...
	outward_normal = vec3_unit(vec3_sub(hit->point,
				vec3_add(tube->center,
					vec3_scale(tube->axis,
						vec3_dot(tube->axis,
							vec3_sub(hit->point, tube->center))))));
