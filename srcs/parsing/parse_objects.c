...
static void	set_default_material(t_material *material, t_vec3 color)
{
	...
	material->u.lambertian.albedo = vec3_div(color, 255);
}
...
static t_object	add_disk(t_tube tube, bool is_top)
{
	...
	axis = tube.axis;
	scaled_axis = vec3_scale(tube.axis, tube.half_height);
	if (is_top)
		axis = vec3_negate(axis);
	else
		scaled_axis = vec3_negate(scaled_axis);
	coord = vec3_sub(tube.center, scaled_axis);
...
bool	parse_cylinder(t_scene *scene, char **line, int *current_object)
{
	...
	if (length != 6
		...
		|| !ft_atod(line[3], &object->u.tube.radius)
		|| object->u.tube.radius <= 0
		|| !ft_atod(line[4], &object->u.tube.half_height)
...
bool	parse_plane(t_scene *scene, char **line, int *current_object)
{
	...
	object->u.plane.material.u.metal.fuzz = 0.2;
...
bool	parse_sphere(t_scene *scene, char **line, int *current_object)
{
	...
	if (length != 4
		...
		|| !ft_atod(line[2], &object->u.sphere.radius)
	...
	object->u.sphere.material.type = MATERIAL_LAMBERTIAN;
	object->u.sphere.material.u.lambertian.albedo
		= vec3_div(object->u.sphere.color, 255);
